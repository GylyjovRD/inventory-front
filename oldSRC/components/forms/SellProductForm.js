...
import { close_icon, check_icon, add_icon, close_icon2, check_icon2 } from "../../assets/iconList";
...
if(postData.items.length === 0){
    ...
    payload:"Siz haryt saylamadynyz"})
} else {
    console.log("post data:", JSON.stringify(postData))
    fetch('/api//products/invoice-create/', {
        method: "POST",
        headers:{
    ...
    .then((res) => res.json())
    .then(json => dispatch({type: CLEAR_INVOICE_PRODUCT}))
    .then(json => dispatch(sellModalClose()))
    .catch((err) => dispatch({type:ERROR_INVOICE_PRODUCT, payload:err}))
}
        ...
        <>
            {invoice.open ? (
                ...
                <div className="relative bg-gray-900 w-screen h-screen z-40 bg-opacity-50">
                    <SellComponent onAddProduct={setAddProduct} addToInvoice={setInvoiceProducts} datas={invoiceProducts} />
                </div>
            ) : (
                <></>
            )
            ...
            <div className={`fixed inset-0 z-30 flex items-end bg-white bg-opacity-50 sm:items-center sm:justify-center`}>
                <div className="w-full h-full px-6 py-4 md:rounded-t-lg bg-gray-800 sm:rounded-lg overflow-scroll">
                    ...
                    <div className="mt-6 mb-6">
                        <div className="flex justify-between items-center">
                            <div className="flex">
                                ...
                                <p className="ml-4 font-semibold text-xl text-white underline underline-offset-2">{invoiceTotal}</p>
                            </div>
                            <span>{invoice.error.isError ? invoice.error.errorMessage : ""}</span>
                            {store && 
                            ...
                            <div className="w-20 flex justify-end">
                                <PrimaryButton title="Gos" icon={add_icon} onPressHandler={() => {
                                    dispatch({type: INVOICE_MODAL_OPEN})
                    ...
                    <footer className="flex fixed bottom-4 flex-col px-6 py-3 -mx-6 -mb-4 space-y-4 sm:space-y-0 sm:space-x-6 sm:flex-row bg-gray-800">
                        ...
                        <PrimaryButton title="Kabul et" icon={check_icon2} onPressHandler={() => invoiceCreateHandler()} />
                    </footer>
                </div>
            </div>
        </>
